...
def search():
    ...
    if not content:
        ...
        generated_content = generate_text_with_gpt2(query, query_type)
        content = generated_content['text']
        images.extend(generated_content['images'])
        related_keywords = generated_content['related_keywords']
...
# Function to generate text and fetch images using GPT-2
def generate_text_with_gpt2(query, query_type):
    input_text = f"Generate {query_type} about {query}"
    
    # Generate text using GPT-2
    generated_content = gpt2_pipeline(input_text, max_length=500, num_return_sequences=1)
    ...
    return {"text": generated_text, "images": images, "related_keywords": extract_keywords(generated_text)}

# Function to extract images related to generated text from the web
def extract_images_from_web(generated_text):
    images = []
    
    # Logic to fetch images based on the generated_text
    for domain in priority_domains:
        url = domain + generated_text
        response = requests.get(url)
        soup = BeautifulSoup(response.content, 'html.parser')
        images.extend([img.get('src') for img in soup.find_all('img', limit=5)])
